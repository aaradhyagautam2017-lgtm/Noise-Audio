...
def sidebar(prefix, active):
    ...
    <aside class="sidebar">
      <div class="appswitcher">
        ...
        <button type="button" class="themebtn" id="theme-toggle" title="Toggle color theme" aria-label="Toggle color theme">
          <span class="theme-ico theme-ico-sun">☀</span><span class="theme-ico theme-ico-moon">☾</span>
        </button>
      </div>
      <nav>
...
/* Dark palette — same variable names, so every rule above re-themes for free with no
   separate dark-mode CSS path. Category colors are brightened (not just inverted) since a
   near-black-on-white color read on black would lose contrast entirely. */
:root[data-theme="dark"]{--bg:#0e0e10;--panel:#1a1a1c;--pearl:#222224;--ink:#f5f5f7;--ink2:#b4b4b8;--ink3:#87878c;--line:#3a3a3c;--line-soft:#28282a;
--accent:#2997ff;--blue:#2997ff;--blue-focus:#409cff;
--warn-bg:#2a2210;--warn-line:#d4922f;--warn-ink:#eec27a;
--atom:#30d158;--molecule:#d9a441;--organism:#ff453a;--complex:#c2453d;--plum:#b39ddb;--blue-tint:#16324a;}
...
.navitem{display:flex;align-items:center;gap:6px;padding:6px 10px;border-radius:var(--r-sm);text-decoration:none;color:var(--ink);font-size:13px;transition:background .12s,color .12s}
.navitem:hover{background:var(--bg)}
.navitem.active{background:var(--blue-tint);color:var(--blue);font-weight:600}
.count{margin-left:auto;background:var(--line-soft);color:var(--ink2);border-radius:var(--r-pill);padding:0 7px;font-size:11px}
.navitem.active .count{background:transparent;color:var(--blue)}
...
.typebadge{display:inline-block;border-radius:var(--r-pill);padding:2px 10px;font-size:11px;font-weight:600;vertical-align:middle}
.t-atom{background:color-mix(in srgb,var(--atom) 16%,transparent);color:var(--atom)}
.t-molecule{background:color-mix(in srgb,var(--molecule) 16%,transparent);color:var(--molecule)}
.t-organism{background:color-mix(in srgb,var(--organism) 16%,transparent);color:var(--organism)}
.t-complex-organism{background:color-mix(in srgb,var(--complex) 18%,transparent);color:var(--complex)}
...
.table th{text-align:left;color:var(--ink3);font-weight:600;font-size:12px;border-bottom:1px solid var(--line);padding:9px 10px}
.table td{border-bottom:1px solid var(--line-soft);padding:9px 10px;vertical-align:middle}
...
.warning{background:var(--warn-bg);border-left:3px solid var(--warn-line);border-radius:var(--r-sm);padding:10px 14px;margin:12px 0;font-size:13px;color:var(--ink2)}
...
.kpirow{display:flex;flex-wrap:wrap;margin:16px 0}
.kpi{border-left:1px solid var(--line);padding:2px 20px;flex:1;min-width:140px}
...
.kpi-warn{border-left-color:var(--warn-line)}
